					...
					<scroll-view scroll-y="true" class="scroll-Hei"  :scroll-with-animation="true" :enhanced="true" :show-scrollbar="false" :scroll-into-view="scroll_into" @scroll="scroLl">
						...
						<view :id="item.cid" class="rig-height">
							...
							<view class="classif-goods" v-for="(itemgood,good_index) in item.good_query" :key="good_index" @click="popup_item(true,index,good_index,item.cid,itemgood)">
								...
								<view class="quantity">
									<view><image v-if="itemgood.quantity > 0" src="/static/tab/jian1.png" mode="widthFix" @click.stop="reduce(index,good_index,item.cid,itemgood)"></image></view>
									<view><text v-if="itemgood.quantity > 0">{{itemgood.quantity}}</text></view>
									<view><image src="/static/tab/jia1.png" mode="widthFix" @click.stop="plus(index,good_index,item.cid,itemgood)"></image></view>
								</view>
							</view>
						</view>
						</block>
			...
			<view class="order-bottom" @click="pop_Shopping()" :style="{'padding-bottom': Modelmes ? '68rpx' : ''}">
				<view class="Shopping" style="width: 115rpx;">
					<view class="Shopping-left">
						<image src="/static/tab/car.PNG" mode="widthFix"></image>
					</view>
...
export default{
	...
	methods:{
		...
		plus(index,good_index,cid,itemgood){
			const {quantity,image,name,unitprice,unit,_id} = itemgood //每份菜品信息
			const QU = quantity + 1  //所选菜品数量加一
			this.$set(this.goods[index].good_query[good_index],'quantity',QU)  //使用this.$set()方法更新quantity数据
			const arr = {image,name,unitprice,quantity:QU,unit,total_price:unitprice * QU,_id,cid,good_index} //使用this.$set()方法更新quantity数据
		...
		shopping_Cart(arr){
			...
			this.qunint_of_goods() //左边各分类下添加了多少菜品总数
		},
		...
		async dishEs(){
			// 请求云函数获取分类和菜品数据
			const res = await wx.cloud.callFunction({name: 'diancanyun',data:{}})
			console.log(res)
			this.itemize = res.result.res_cate//类目
			this.goods = res.result.res_dis//所有菜品
			this.$nextTick(()=>{
				this.goods_height()  // 计算右边每个分类菜品的高度的方法
			})
		},
		// 计算右边每个分类菜品的高度
		goods_height(){
		...
		placean_order(){
			...
			wx.requestSubscribeMessage({  //微信官方文档的订阅消息接口
			  ...
			  success:(res)=>{
				  this.sub_database() //如果选择订阅，提交订单
			  },
			  fail:(err)=>{
				  this.sub_database() //如果选择不订阅，提交订单
			  }
			})
		},
...
<style scoped>
.top-view{
	background:linear-gradient(to top, #82ce8e,#78ce7d,#71ce79,#4CCE61);
	height: 120rpx;
	display: flex;
...
.place-order button{
	border: none;
	background:linear-gradient(to right,#82ce8e,#78ce7d,#71ce79,#4CCE61);
	width: 200rpx;
	height: 75rpx;
	line-height: 75rpx;
	border-radius: 50rpx;
