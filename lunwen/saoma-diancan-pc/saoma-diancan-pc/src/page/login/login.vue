...
export default{
	...
	methods:{
		...
		async register(){
			this.load = true 
			let obj = {account:this.account,password:this.password}  //获取输入的账号信息
			try{
				let res = await new this.Request(this.Urls.m().register,this.$qs.stringify(obj)).modepost() //调用注册接口，提交账号信息
				if(res.status != 200){ //提交失败
					new this.mytitle(this.$message,'warning',res.data.msg).funtitle() //显示提交失败信息
				}else if(res.status == 200){ //提交成功
					this.regi = '注册'  //显示注册成功
				}
				this.load = false
			}catch(e){
		...
		async signin(){
			...
			let obj = {account:this.account,password:this.password} //获取输入的账号信息
			try{
				let res = await new this.Request(this.Urls.m().login,this.$qs.stringify(obj)).modepost()//调用登录接口，提交账号信息
				if(res.status != 200){ //提交失败
					// console.log('123')
					new this.mytitle(this.$message,'warning',res.data.msg).funtitle()//显示提交失败信息
				}else if(res.status == 200){ //提交成功
					let ids = '1'
					localStorage.setItem("nuvmenuid", JSON.stringify(ids)) //创建本地存储
...
<style scoped="scoped">
#backcont {
	background-image: url(../../../static/login/2.jpg);
