...
router.post('/qrcode', new Auth().m, async ctx=>{
	...
	let time = moment().utcOffset(8).format('YYYY-MM-DD HH:mm:ss') //生成时间
	try{
		// 查询该桌号是否已存在
		const query = `db.collection('table_qr_code').where({table:'${table}'}).get()`
		const res = await new getToken().posteve(Tripurl,query) //调用数据库查询接口
		if(res.data.length > 0){
			new result(ctx,'该桌号已经存在',202).answer()
		}else{
			let res_code = await new getToken().qrcode(table)
			const res_image = await buffer(Code(),res_code.data) //图片上传
			let code_image = 'https://' + res_image // 给二进制图片重新命名
			let table_data = `db.collection('table_qr_code').add({data:{time:'${time}',table:'${table}',code:'${code_image}'}})`//使用数据库指令添加生成的桌号
			await new getToken().posteve(Addurl,table_data) //调用数据库添加接口添加桌号
