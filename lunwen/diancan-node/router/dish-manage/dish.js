...
router.post('/uploaddishes', new Auth().m, async ctx=>{
	const {id,category,name,unitprice,unit,image,value} = ctx.request.body //获取菜品信息
	// 校验
	...
	try{
		await new getToken().posteve(Addurl,query)  //调用数据库的Addurl接口 实现数据增加
		await new getToken().posteve(Updateurl,count) //调用数据库的Updateurl接口 实现数据更新
		new result(ctx,'提交成功').answer() //返回提交成功信息
	}catch(e){
		new result(ctx,'提交失败,服务器发生错误',500).answer() //返回提交失败信息
	}
})
